...
type DriverParameters struct {
	...
	ReadFallbackAccessKey       string
	ReadFallbackSecretKey       string
	ReadFallbackSessionToken    string
	ReadFallbackBucket          string
	ReadFallbackRegion          string
	ReadFallbackRootDirectory   string
}
...
func FromParameters(parameters map[string]interface{}) (*Driver, error) {
	...
	readFallbackAccessKey := parameters["readfallbackaccesskey"]
	if readFallbackAccessKey == nil {
		readFallbackAccessKey = ""
	}
	readFallbackSecretKey := parameters["readfallbacksecretkey"]
	if readFallbackSecretKey == nil {
		readFallbackSecretKey = ""
	}
	readFallbackSessionToken := parameters["readfallbacksessiontoken"]
	if readFallbackSessionToken == nil {
		readFallbackSessionToken = ""
	}
	readFallbackRegionName := parameters["readfallbackregion"]
	if readFallbackRegionName == nil {
		readFallbackSessionToken = ""
	}
	readFallbackRegion := fmt.Sprint(readFallbackRegionName)
	if readFallbackRegion != "" {
		if  _, ok := validRegions[readFallbackRegion]; !ok {
			return nil, fmt.Errorf("invalid region provided: %v", readFallbackRegion)
		}
	}
	readFallbackBucket := parameters["readfallbackbucket"]
	if readFallbackBucket == nil || fmt.Sprint(readFallbackBucket) == "" {
		return nil, fmt.Errorf("no bucket parameter provided")
	}
	readFallbackRootDirectory := parameters["readfallbackrootdirectory"]
	if rootDirectory == nil {
		rootDirectory = ""
	}
	...
	params := DriverParameters{
		...
		fmt.Sprint(readFallbackAccessKey),
		fmt.Sprint(readFallbackSecretKey),
		fmt.Sprint(readFallbackSessionToken),
		fmt.Sprint(readFallbackBucket),
		readFallbackRegion,
		fmt.Sprint(readFallbackRootDirectory),
	}
...
func New(params DriverParameters) (*Driver, error) {
	...
	// Create read fallback S3 obj.
	fallbackAWSConfig := aws.NewConfig()
	fallbackSess, err := session.NewSession()
	if err != nil {
		return nil, fmt.Errorf("failed to create new session: %v", err)
	}
	fallbackCreds := credentials.NewChainCredentials([]credentials.Provider{
		&credentials.StaticProvider{
			Value: credentials.Value{
				AccessKeyID:     params.ReadFallbackAccessKey,
				SecretAccessKey: params.ReadFallbackSecretKey,
				SessionToken:    params.ReadFallbackSessionToken,
			},
		},
		&credentials.EnvProvider{},
		&credentials.SharedCredentialsProvider{},
	...
	fallbackAWSConfig.WithCredentials(fallbackCreds)
	fallbackAWSConfig.WithRegion(params.ReadFallbackRegion)
	fallbackSess, err = session.NewSession(fallbackAWSConfig)
	if err != nil {
		return nil, fmt.Errorf("failed to create new session with aws config: %v", err)
	}
	fallbackS3obj := s3.New(fallbackSess)

	if !params.V4Auth {
		setv2Handlers(fallbackS3obj)
	}
	...
	d := &driver{
		...
		ReadFallbackS3:				 fallbackS3obj,
		ReadFallbackBucket:          params.ReadFallbackBucket,
		ReadFallbackRootDirectory:   params.ReadFallbackRootDirectory,
	}
...
func (d *driver) Reader(ctx context.Context, path string, offset int64) (io.ReadCloser, error) {
	...
	resp, err = d.ReadFallbackS3.GetObject(&s3.GetObjectInput{
		Bucket: aws.String(d.ReadFallbackBucket),
		Key:    aws.String(d.s3ReadFallbackPath(path)),
		Range:  aws.String("bytes=" + strconv.FormatInt(offset, 10) + "-"),
	})
	if err != nil {
...
func (d *driver) Stat(ctx context.Context, path string) (storagedriver.FileInfo, error) {
	...
	if err != nil {
		return d.statReadFallback(ctx, path)
	}
...
func (d *driver) statReadFallback(ctx context.Context, path string) (storagedriver.FileInfo, error) {
	resp, err := d.ReadFallbackS3.ListObjects(&s3.ListObjectsInput{
		Bucket:  aws.String(d.ReadFallbackBucket),
		Prefix:  aws.String(d.s3ReadFallbackPath(path)),
		MaxKeys: aws.Int64(1),
	})
	if err != nil {
		return nil, err
	...
	if len(resp.Contents) == 1 {
		if *resp.Contents[0].Key != d.s3ReadFallbackPath(path) {
			fi.IsDir = true
		} else {
			fi.IsDir = false
			fi.Size = *resp.Contents[0].Size
...
func (d *driver) s3ReadFallbackPath(path string) string {
	return strings.TrimLeft(strings.TrimRight(d.ReadFallbackRootDirectory, "/")+path, "/")
}
